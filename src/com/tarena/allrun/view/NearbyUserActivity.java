...
public class NearbyUserActivity extends BaseActivity {
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == 100 && resultCode == 200) {
			// 说明data来之addFriendBiz不是来自系统图库的图片
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			this.registerReceiver(showNearbyUserReceiver, new IntentFilter(Const.ACTION_GET_NEARBY_USER));
	...
	private void setupView() {
		...
		pullToRefreshListView.setOnScrollListener(new PauseOnScrollListener(bitmapUtils, false, true));
	}

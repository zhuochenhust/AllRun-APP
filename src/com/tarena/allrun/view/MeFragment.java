...
public class MeFragment extends Fragment {
	...
	public Handler handler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			try {
				...
				case SHOW_VERSION_INFO:
					VersionEntity versionEntity = (VersionEntity) bundle.getSerializable(Const.KEY_DATA);
		...
		private void showDialog(final VersionEntity versionEntity) throws Exception {

			// 判断版本号是不是最新的
			String currentVersion = Tools.getCurrentVersion(getActivity());
			if (Double.parseDouble(currentVersion) < Double.parseDouble(versionEntity.getNewVersion())) {
				// showDialog
				AlertDialog.Builder dialog = new Builder(getActivity());
				dialog.setMessage(versionEntity.getNewVersion() + "\n" + versionEntity.getChangeLog());

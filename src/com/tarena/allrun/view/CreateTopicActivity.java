...
public class CreateTopicActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			this.registerReceiver(myReceiver, new IntentFilter(Const.ACTION_GET_Address));
			this.registerReceiver(myReceiver, new IntentFilter(Const.ACTION_CREATE_TOPIC_OVER));
		} catch (Exception e) {
		...
		try {
			...
			tvSubmit.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					try {
						address = null;
						Tools.showProgressDialog(CreateTopicActivity.this, "正在创建话题");

						// 1:发坐标到百度云服务器查询地址

						TopicBiz.queryAddress(String.valueOf(latitude), String.valueOf(longitude));
								...
								try {
									...
									runOnUiThread(new Runnable() {
										...
										public void run() {
											...
											TopicBiz.createTopic(TApplication.currentUser, body, imageData, address,
													String.valueOf(latitude), String.valueOf(longitude), time);
			...
			try {
				...
				if (Const.ACTION_CREATE_TOPIC_OVER.equals(action)) {
					...
					try {
						...
						CreateTopicActivity.this.setResult(RESULT_OK, data);

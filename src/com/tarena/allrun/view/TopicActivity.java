...
public class TopicActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			this.registerReceiver(topicReceiver, new IntentFilter(Const.ACTION_SHOW_TOPIC));
	...
	private void addListener() {
		tvCreateTopic.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				try {
					startActivityForResult(new Intent(TopicActivity.this, CreateTopicActivity.class), 100);
				} catch (Exception e) {
					ExceptionUtil.handleException(e);
				}
			}
	...
	class TopicReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			try {
				...
				if (isRefresh) {
					ArrayList<TopicEntity> list = (ArrayList<TopicEntity>) intent.getSerializableExtra(Const.KEY_DATA);

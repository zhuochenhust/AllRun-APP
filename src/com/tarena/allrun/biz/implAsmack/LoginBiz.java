		...
		new Thread("login thread") {
			public void run() {
				...
				try {
					...
					Log.i("loginBiz",
							"login thread " + threadId + "time=" + (startLoginTime - TApplication.appStartTime));
					...
					// 2:while
					int count = 0;
					while (count < 6000 && TApplication.xmppConnection.isConnected() == false) {
						count++;
						this.sleep(10);
					}
					// 再次判断是否连接上
					...
					if ("SASL authentication failed using mechanism DIGEST-MD5: ".equals(info)) {
						status = Const.STATUS_PASSWORD_ERROR;

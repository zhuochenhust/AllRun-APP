...
public class RegisterBiz extends IntentService {
	...
	protected void onHandleIntent(Intent intent) {
		try {
			...
			// 把注册信息发给tomcat
			// 1,提交给谁
			String url = "http://" + TApplication.host + ":8080/allRunServer/register.jsp";
			// 2,发什么数据
			// httpClient,asyncHttpClient,java.net.HttpUrlConnection
			HttpUtils httpUtils = new HttpUtils(60000);
			RequestParams requestEntity = new RequestParams();
			requestEntity.addBodyParameter("username", userEntity.getUsername());
			...
			requestEntity.addBodyParameter("file", byteArrayInputStream, byteArrayInputStream.available(), fileName,
					"image/png");

			httpUtils.send(HttpMethod.POST, url, requestEntity, new RequestCallBack<String>() {
				...
				public void onFailure(HttpException error, String msg) {
					// 发广播
					Intent intent2 = new Intent(Const.ACTION_REGISTER);
					intent2.putExtra(Const.KEY_DATA, Const.STATUS_REGISTER_FAILURE);
					sendBroadcast(intent2);
				}
			});

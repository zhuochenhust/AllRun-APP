		...
		new Thread("connect thread") {
			public void run() {
				try {
					...
					Log.i("loginBiz", "connect thread " + threadId + "time=" + (connectOverTime - appStartTime));

					Log.i("connectChatServer", "连接服务器结果" + xmppConnection.isConnected());
				} catch (Exception e) {
					// TODO: handle exception
				} finally {
	...
	class AllPackeInterceptor implements PacketInterceptor {
		...
		public void interceptPacket(Packet packet) {
			try {
				...
				LogUtil.i("AllPackeInterceptor", "classInfo=" + classInfo + "  xmlString=" + xmlString);
			} catch (Exception e) {
				// TODO: handle exception
			}
	...
	class AllPacketListener implements PacketListener {
		...
		public void processPacket(Packet packet) {
			try {
				...
				String classInfo = packet.toString();
				String xmlString = packet.toXML();
				LogUtil.i("AllPacketListener", "classInfo=" + classInfo + "  xmlString=" + xmlString);
				if (packet instanceof RosterPacket) {
					// RosterPacket:放的是当前用户的好友信息
					RosterPacket rosterPacket = (RosterPacket) packet;
					ArrayList<Item> list = new ArrayList(rosterPacket.getRosterItems());
					for (Item item : list) {
				...
				if (packet instanceof Message) {
					...
					if (type == Type.chat) {
						...
						if (friendUser.contains("/")) {
							...
							sendBroadcast(new Intent(Const.ACTION_SHOW_GROUP_CHAT_MESSAGE));
						}
